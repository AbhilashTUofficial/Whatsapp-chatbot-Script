(() => {
    ...
    const start = (_chats, cnt = 0) => {
        ...
        } else if (lastMessageOnChat != getLastMsg() && getLastMsg() !== false && (!didYouSendLastMsg() || didYouSendLastMsg())){
        ...
        if (lastMsg.toUpperCase().indexOf('@KOMALLAN') > -1){
            sendText = `
				Cool ${title}! ethill ethekillum Aayakk:
				1. *@TIME*
				2. *@JOKE*`
        }
